...
export function MovieCard({ movie, onViewDetails }: { movie: Movie; onViewDetails: (m: Movie) => void }) {
  ...
  return (
    ...
    >
      {/* 🏎️ ACTIVE INFINITE SILVER LIGHT LOOP TRACER OVERLAY */}
      <AnimatePresence>
        {isHovered && (
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="absolute inset-0 z-30 pointer-events-none rounded-2xl border-2 border-transparent"
            style={{
              background: 'linear-gradient(#0d1222, #0d1222) padding-box, conic-gradient(from 0deg, transparent 20%, #ffffff 50%, transparent 80%) border-box',
